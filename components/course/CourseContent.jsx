...
class CourseContent extends React.Component {
  ...
  getPractices() {
    HttpServices.request('getPractices', { filter: { course: this.state.id } }, (fetchResult, fetchError) => {
      ...
      // console.info('getOneUserCourse==========',fetchResult)
  ...
  getOneUserCourse() {
    ...
    HttpServices.request('getOneUserCourse', { course: this.state.id }, (fetchResult, fetchError) => {
      ...
      console.info('getOneUserCourse==========',fetchResult)
      this.setState({
        userCourse: fetchResult?.info, watchedLessons: fetchResult?.info?.watchedLessons?.length ?? 0, donePractices: fetchResult?.info?.donePractices?.length ?? 0
        
      }, () => {
        // console.info('getOneUserCourse==========watchedLessons',this.state.watchedLessons)
        // console.info('getOneUserCourse==========',this.state.donePractices)
        
        // // let found = false
        // let diff = 0
        // console.info('getOneUserCourse==========',this.state.donePractices)
        // for (let i = 0; i < this.state.watchedLessons; i++) {
        //   const element = this.state.lessons[i];
        //   if(element._id == this.state.practices[(this.state.donePractices)].lesson._id){
        //     // found = true
        //     break;
        //   }else{
        //     diff = diff + 1
        ...
        //   this.setState({donePractices : this.state.donePractices + diff, diff},()=>{
        //     console.info('getOneUserCourse==========',this.state.donePractices, diff)
          ...
          {this.state.lessons?.map((item, index) => {
            return (
              <>
                {item.locked ? (
                  ...
                  <>
                    {(!item.tempLock) ? (
                      ...
                      <div key={index} className="col-xl-12 box-9 d-flex flexcb px-3 py-3 mb-2 ">
                        <div className="d-flex">
                          <div className="d-none d-lg-flex">
                            <p className=" pr-2  text-semibig">اسنک {index + 1}: </p>
                          </div>
                ...
                {this.state.practices?.map((practice, pIndex) => {
                  if (practice.lesson._id == item._id) {

                    return (
                      <>
                        {(!practice.locked || this.props.user.info.status == "admins" ) ? (
                            <Link href={'/practice/' + practice?._id} >
                            <a>


                          <div key={pIndex} className="col-xl-12 box-3 d-flex flexcb px-3 py-3 mb-2 " style={{ textAlign: 'start' }}>
                            {/* <div className="d-flex"> */}

                            <div className=" text-color-2">
                              <p className="">{practice.title}</p>
                            </div>
                            {/* <div className="d-none d-xl-block col-xl-4 text-color-2">
                            <p>{practice.lesson.title}</p>
                          </div> */}

                            {/* </div> */}
                            <div className="" >
                              <div className=" flexc  " style={{ justifyContent: 'flex-end' }}>
                                <p style={{ whiteSpace: 'nowrap' }} className="text-color-2 text-semibig">{msToHMS((practice?.time * 60) ?? 0)} </p>
                          ...
                          <div key={pIndex} className="col-xl-12 box-9 d-flex flexcb px-3 py-3 mb-2 " style={{ textAlign: 'start' }}>
                            {/* <div className="d-flex"> */}
