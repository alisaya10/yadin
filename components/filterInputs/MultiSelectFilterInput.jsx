...
class MultiSelectFilterInput extends React.Component {
    ...
    changeValue() {
        ...
        this.state.choosen.forEach(element => {
            let id = (typeof element.value == 'object' ? element.value?._id : element.value)
            choosen.push(id)
            dictionary[id] = element.title
            // choosen.push((element.value))
        });
        // console.log(dictionary)

        this.props.changeValue(this.props.header.key, choosen, { type: this.props.header.type, operator: this.state.operator, label: this.props.header.information?.label, dictionary })
    }
    ...
    render() {
        ...
        return (
            <div ref={ref => this.setWrapperRef(ref)} className='mt-0 w-100 h-100 flexc' style={{ position: 'relative' }} onClick={() => this.toggleOpen(true)} >
